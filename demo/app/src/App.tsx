...
import { loadSecretKey, getBalances, getSolBalance, requestAirdrop, payAndFetch, type Step, type Balances } from './wallet.js'
...
function Playground() {
  ...
  const [feePayerInfo, setFeePayerInfo] = useState<{ address: string; balance: number } | null>(null)
      ...
      switch (step.type) {
        ...
        case 'challenge':
          if (step.feePayerKey && !feePayerInfo) {
            getSolBalance(step.feePayerKey).then((bal) =>
              setFeePayerInfo({ address: step.feePayerKey!, balance: bal })
            ).catch(() => {})
          }
          const decimals = step.currency === 'SOL' ? 9 : 6
          const human = (Number(step.amount) / 10 ** decimals).toFixed(decimals === 9 ? 4 : 2)
  ...
  if (!ready) {
    return <WalletSetup onReady={() => { setReady(true); refreshBalance() }} />
  }
  ...
  const sidebar = (
    <div style={s.sidebar}>
      <div style={s.sidebarHeader}>
        <span style={{ fontSize: 14, fontWeight: 700, color: '#fff' }}>Endpoints</span>
        <button style={s.walletBtn} onClick={() => setShowWallet(true)}>wallet</button>
      </div>
      {ENDPOINTS.map((ep, i) => (
        <button
          key={ep.path}
          ...
          onClick={() => { setSelectedIdx(i); setParamValues({}) }}
        >
          <span style={{ color: ep.method === 'GET' ? '#14F195' : '#FFD700', fontSize: 10 }}>
            {ep.method}
          </span>
          <span style={{ color: '#ccc', fontSize: 12, marginLeft: 8 }}>{ep.description}</span>
          <span style={{ color: '#666', fontSize: 10, marginLeft: 'auto' }}>{ep.cost}</span>
        </button>
      ))}
        ...
        {feePayerInfo && (
          <div style={s.balanceSection}>
            ...
            <div style={{ color: '#555', fontSize: 9, marginTop: 2 }}>
              {feePayerInfo.address.slice(0, 8)}...{feePayerInfo.address.slice(-4)}
            </div>
          </div>
        )}
  ...
  const apiPanel = (
    <div style={s.apiPanel}>
      <div style={s.apiHeader}>
        <span style={{ color: '#14F195', fontSize: 11 }}>{endpoint.method}</span>
        <span style={{ color: '#ccc', fontSize: 13, marginLeft: 8 }}>{endpoint.path}</span>
        <span style={{ color: '#666', fontSize: 11, marginLeft: 'auto' }}>{endpoint.cost}</span>
      </div>
      <div style={s.params}>
        {(endpoint.params ?? []).map((p) => (
          <div key={p.name} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            <label style={{ color: '#888', fontSize: 12, minWidth: 60 }}>{p.name}</label>
            <input
              style={s.input}
      ...
      <div style={{ display: 'flex', gap: 8, padding: '0 16px 16px' }}>
        ...
        <button
          style={{ ...s.codeToggle, borderColor: showCode ? '#9945FF' : '#333' }}
  ...
  const terminal = (
    <div ref={logRef} style={s.terminal}>
      ...
      {logs.map((log) => (
        <div key={log.id} style={{ padding: '2px 16px', fontSize: 12, color: kindColor[log.kind], whiteSpace: 'pre-wrap', wordBreak: 'break-all' }}>
          {log.text}
        </div>
      ))}
    </div>
      ...
      <div style={{ height: '100vh', display: 'flex', flexDirection: 'column' }}>
        <div style={s.mobileTabs}>
          {(['api', 'terminal', 'code'] as const).map((tab) => (
            <button
              key={tab}
              style={{ ...s.mobileTab, borderBottomColor: mobileTab === tab ? '#9945FF' : 'transparent' }}
        ...
        <div style={{ flex: 1, overflow: 'auto' }}>
          {mobileTab === 'api' && <>{sidebar}{apiPanel}</>}
        ...
        {showWallet && <WalletModal onClose={() => setShowWallet(false)} onReset={() => setReady(false)} />}
      </div>
    )
  }
